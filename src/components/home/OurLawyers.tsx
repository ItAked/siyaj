                    ...
                    <div className="py-12 md:py-20">
                        ...
                        <div className="carousel w-full">
                            {Array.from({ length: Math.ceil(lawyers.length / 3) }).map((_, index) => {
                                ...
                                return (
                                    <div key={`item${index + 1}`} id={`item${index + 1}`} className="carousel-item grid grid-cols-3 w-full gap-x-0 place-items-center">
                        ...
                        <div className="flex w-full justify-center gap-2 py-2">
                            {Array.from({ length: Math.ceil(lawyers.length / 3) }).map((_, index) => (
                                <a key={`nav${index + 1}`} href={`#item${index + 1}`} className={`btn ${activePage === index + 1 ? 'w-20' : 'w-5'} btn-xs rounded-full
                                    ${activePage === index + 1 ? 'bg-gray-900' : 'bg-gray-300'}`} onClick={() => setActivePage(index+1)}></a>
                            ))}
                        </div>
                    </div>
                </div>
