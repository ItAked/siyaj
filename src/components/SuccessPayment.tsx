...
export default function SuccessPayment({
  ...
  const createConfetti = () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      confettiArray.push(
        <div
          key={i}
          className={`absolute w-2 h-2 ${randomColor} animate-bounce opacity-0`}
          style={{
            left: `${randomLeft}%`,
            animationDelay: `${randomDelay}s`,
            animationDuration: `${randomDuration}s`,
            animation: `confetti-fall ${randomDuration}s linear infinite ${randomDelay}s`,
          }}
        />,
      )
    }
  ...
  return (
    <>
      ...
      <div
        className="min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-purple-700 flex items-center justify-center p-5"
        dir="rtl"
      >
        <div className="bg-white rounded-3xl p-12 md:p-16 text-center shadow-2xl max-w-lg w-full relative overflow-hidden animate-slideUp">
          ...
          {/* Thank you text */}
          <h1 className="text-5xl md:text-6xl font-bold text-gray-800 mb-5 animate-fadeInUp-1 drop-shadow-sm">
            شكرًا لك
          </h1>

          {/* Success message */}
          <p className="text-lg text-gray-700 mb-10 leading-relaxed animate-fadeInUp-3">
            تم إتمام عملية الدفع بنجاح. سيتم إرسال تأكيد الطلب إلى بريدك الإلكتروني قريباً.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fadeInUp-5">
            <button
              ...
              className={`px-8 py-3 rounded-lg font-semibold transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg ${
                downloadStatus
            ...
            <button
              onClick={goHome}
              className="px-8 py-3 bg-gray-600 hover:bg-gray-700 text-white rounded-lg font-semibold transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg hover:shadow-gray-500/30"
            >
